...
double cutoff = 80; //curoff voltage
double k_p = 80; //proportional gain
double k_d = 2.5; //derivative gain
int Sensor_R = 40;
int Sensor_L = 42;
int error = 0;
int con = 0;
...
void loop()
{
  ...
  while (!stopbutton()) {
    ...
    if ((sens_L > cutoff) && (sens_R > cutoff)) error = 0;
    if ((sens_L > cutoff) && (sens_R < cutoff)) error = 1;
    if ((sens_L < cutoff) && (sens_R > cutoff)) error = -1;
